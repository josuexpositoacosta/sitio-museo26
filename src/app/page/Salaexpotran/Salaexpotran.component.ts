...
@Component({
  selector: 'app-Salaexpotran',
  templateUrl: './Salaexpotran.component.html',
  styles: [
  ]
})
export class SalaexpotranComponent implements OnInit {
  paginis: Pagini[] = []; 
  url!: String;
  ...
  cargarurl(i: number): any{   
    var url = `${URL_SERVICIOS}/img/paginis/${this.paginis[i].img}`; // URL_SERVICIOS + '/img/paginis/' + this.paginis[i].img;
  //  this._paginiService.cargarurl(this.paginis[i])
    return url;
   // .subscribe(() =>{                    
     //    this.paginis = resp.paginis;
 //  });
